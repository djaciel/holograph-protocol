...
contract('Governor', function (accounts) {
  ...
  shouldSupportInterfaces(['ERC165', 'Governor', 'GovernorWithParams']);
  ...
  describe('scenario', function () {
    ...
    describe('vote with signature', function () {
      beforeEach(async function () {
        ...
        this.settings = {
          ...
          tokenHolder: owner,
          voters: [{ voter: this.voter, signature, weight: web3.utils.toWei('10'), support: Enums.VoteType.For }],
        };
      });
      afterEach(async function () {
        expect(await this.mock.hasVoted(this.id, owner)).to.be.equal(false);
    ...
    describe('send ethers', function () {
      beforeEach(async function () {
        ...
        this.settings = {
          proposal: [[this.receiver.address], [this.value], ['0x'], '<proposal description>'],
          tokenHolder: owner,
          voters: [
    ...
    describe('receiver revert without reason', function () {
      beforeEach(async function () {
        this.settings = {
          ...
          tokenHolder: owner,
          voters: [{ voter: voter1, weight: web3.utils.toWei('10'), support: Enums.VoteType.For }],
          steps: {
    ...
    describe('score not reached', function () {
      beforeEach(async function () {
        this.settings = {
          ...
          tokenHolder: owner,
          voters: [{ voter: voter1, weight: web3.utils.toWei('10'), support: Enums.VoteType.Against }],
          steps: {
            execute: { error: 'Governor: proposal not successful' },
          },
        };
  ...
  describe('Proposal length', function () {
    it('empty', async function () {
      await expectRevert(this.mock.propose([], [], [], '<proposal description>'), 'Governor: empty proposal');
    });
    ...
    it('missmatch #3', async function () {
      await expectRevert(
        this.mock.propose([this.receiver.address], [web3.utils.toWei('0')], [], '<proposal description>'),
        'Governor: invalid proposal length',
      );
    });
  });
  ...
  describe('Settings update', function () {
    describe('setVotingDelay', function () {
      ...
      afterEach(async function () {
        ...
        expectEvent(this.receipts.execute, 'VotingDelaySet', { oldVotingDelay: '4', newVotingDelay: '0' });
      });
      runGovernorWorkflow();
    });
    ...
    describe('setVotingPeriod', function () {
      ...
      afterEach(async function () {
        ...
        expectEvent(this.receipts.execute, 'VotingPeriodSet', { oldVotingPeriod: '16', newVotingPeriod: '32' });
      });
      runGovernorWorkflow();
    });
    ...
    describe('setProposalThreshold', function () {
      ...
      afterEach(async function () {
        ...
        expectEvent(this.receipts.execute, 'ProposalThresholdSet', {
