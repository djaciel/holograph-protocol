...
contract('TimelockController', function (accounts) {
  ...
  beforeEach(async function () {
    // Deploy new timelock
    this.timelock = await TimelockController.new(MINDELAY, [proposer], [executor], { from: admin });
    ...
    describe('simple', function () {
      describe('schedule', function () {
        ...
        it('proposer can schedule', async function () {
          ...
          expect(await this.timelock.getTimestamp(this.operation.id)).to.be.bignumber.equal(
  ...
  describe('maintenance', function () {
    it('prevent unauthorized maintenance', async function () {
      await expectRevert(this.timelock.updateDelay(0, { from: other }), 'TimelockController: caller must be timelock');
    });
