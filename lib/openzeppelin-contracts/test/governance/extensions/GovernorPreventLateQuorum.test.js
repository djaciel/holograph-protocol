...
contract('GovernorPreventLateQuorum', function (accounts) {
  ...
  describe('nominal is unaffected', function () {
    ...
    afterEach(async function () {
      ...
      this.receipts.castVote.filter(Boolean).forEach((vote) => {
        ...
        expectEvent.notEmitted(vote, 'ProposalExtended');
      });
  ...
  describe('Delay is extended to prevent last minute take-over', function () {
    ...
    afterEach(async function () {
      ...
      expectEvent(tx, 'ProposalExtended', { proposalId: this.id, extendedDeadline: extendedBlock });
  ...
  describe('setLateQuorumVoteExtension', function () {
    ...
    it('protected', async function () {
      await expectRevert(this.mock.setLateQuorumVoteExtension(this.newVoteExtension), 'Governor: onlyGovernance');
    });
    ...
    describe('using workflow', function () {
      beforeEach(async function () {
        this.settings = {
          ...
          proposer,
          tokenHolder: owner,
          voters: [{ voter: voter1, weight: web3.utils.toWei('1.0'), support: Enums.VoteType.For }],
        };
      });
      afterEach(async function () {
        expectEvent(this.receipts.propose, 'ProposalCreated', { proposalId: this.id });
        expectEvent(this.receipts.execute, 'ProposalExecuted', { proposalId: this.id });
        expectEvent(this.receipts.execute, 'LateQuorumVoteExtensionSet', {
          oldVoteExtension: lateQuorumVoteExtension,
          newVoteExtension: this.newVoteExtension,
        });
