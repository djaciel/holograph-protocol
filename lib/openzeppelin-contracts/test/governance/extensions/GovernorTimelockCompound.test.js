...
contract('GovernorTimelockCompound', function (accounts) {
  ...
  describe('nominal', function () {
    ...
    afterEach(async function () {
      ...
      await expectEvent.inTransaction(this.receipts.queue.transactionHash, this.timelock, 'QueueTransaction', {
        eta: this.eta,
      });
      expectEvent(this.receipts.execute, 'ProposalExecuted', { proposalId: this.id });
      await expectEvent.inTransaction(this.receipts.execute.transactionHash, this.timelock, 'ExecuteTransaction', {
        eta: this.eta,
      });
  ...
  describe('cancel after queue prevents executing', function () {
    ...
    afterEach(async function () {
      ...
      await expectEvent.inTransaction(receipt.receipt.transactionHash, this.timelock, 'CancelTransaction');
  ...
  describe('relay', function () {
    beforeEach(async function () {
      ...
      this.call = [this.token.address, 0, this.token.contract.methods.transfer(other, 1).encodeABI()];
    });

    it('protected', async function () {
      await expectRevert(this.mock.relay(...this.call), 'Governor: onlyGovernance');
    });
  ...
  describe('transfer timelock to new governor', function () {
    ...
    describe('using workflow', function () {
      ...
      afterEach(async function () {
        expectEvent(this.receipts.propose, 'ProposalCreated', { proposalId: this.id });
        expectEvent(this.receipts.execute, 'ProposalExecuted', { proposalId: this.id });
        await expectEvent.inTransaction(this.receipts.execute.transactionHash, this.timelock, 'NewPendingAdmin', {
