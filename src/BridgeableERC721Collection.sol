...
import "./BridgeableContract.sol";
import "./SecureStorage.sol";

import "./external/OpenSea.sol";
import "./interface/IERC721Receivable.sol";
...
/**
 * @title Bridgeable Collection
 * @author CXIP-Labs
 * @notice A smart contract for minting and managing Bridgeable ERC721 NFTs.
 * @dev The entire logic and functionality of the smart contract is self-contained.
 */
contract BridgeableERC721Collection is Admin {
   ...
   /**
            ...
            abi.encodePacked(
                ...
                Base64.encode(
                    abi.encodePacked(
                        ...
                        Strings.uint2str(_collectionData.bps),
                        ",\"fee_recipient\":\"",
    ...
    function init(address newOwner, CollectionData calldata collectionData) public {
        require(Address.isZero(getAdmin()), "CXIP: already initialized");
        setAdmin(msg.sender);
    ...
    function safeTransferFrom(address from, address to, uint256 tokenId, bytes memory data) public payable {
        ...
        if (Address.isContract(to)) {
            require(
                IERC721Receivable(to).onERC721Received(address(this), from, tokenId, data) == 0x150b7a02,
    ...
    function isOwner() public view returns (bool) {
        return (msg.sender == _owner || msg.sender == getAdmin());
    }
     ...
     */
    fallback() external {
        // IT IS IMPORTANT TO UNDERSTAND THAT STORAGE WOULD BE A SEPARATE CONTRACT AND ACCESED VIA CALL NOT DELEGATECALL
        // THIS IS THE ONLY TRUE SECURE WAY TO PREVENT MODIFICATION OR MANIPULATION OF THE UNDERLYING DATA
        // here the important part we need is to have a storage address specifiable
        // then we call storage and lock it in before allowing any fallbacks
        _storage().lock(true);
        // real fallback here
        _storage().lock(false);
    }
    ...
    function _storage() internal view returns (SecureStorage) {
        return SecureStorage(BridgeableContract(payable(address(this))).getSecureStorage());
    }
